            ...
            <div class="content">
                ...
                <div class="page-header justify-content-between">
                    <div class="page-title">
                        ...
                        <h6>Manage Processing Transfer</h6>
                    </div>
                    <div class="page-btn">
                                ...
                                <dynamic-data-table :headers="headers" :items="items" :loading="loading"
                                    ...
                                    <template #item-actions="item">
                                        <div class="table-actions d-flex gap-2">
                                            <router-link
                                                :to="{ name: 'view-transfer', params: { id: item.stfNo }, query: { backPath: '/stock-transfer/processing-transfer', backLabel: 'Processing Transfer' } }"
                                                class="btn btn-sm btn-icon-only btn-outline-secondary" title="View">
                                                <vue-feather type="eye" size="14"></vue-feather>
                                            </router-link>
                                            <button type="button" class="btn btn-sm btn-icon-only btn-outline-info"
                                                title="Print" data-bs-toggle="modal" data-bs-target="#print-item"
                                            ...
                                            <button v-if="item.isOwner" type="button"
                                                class="btn btn-sm btn-icon-only btn-outline-danger" title="Cancel"
                                                data-bs-toggle="modal" data-bs-target="#cancel-transfer"
        ...
        async fetchTransfers() {
            ...
            try {
                ...
                this.items = allTransfers.filter(
                    t => t.status && t.status.toLowerCase() === 'processing'
                );
                console.log("Processing transfers fetched:", this.items);
            } catch (error) {
                console.error("Failed to fetch processing transfers:", error);
            } finally {
                this.loading = false;
            }
        },
