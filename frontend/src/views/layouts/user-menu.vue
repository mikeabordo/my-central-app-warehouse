      ...
      this.sideBarData.forEach((item) => {
        if (item.menu) {
          item.menu.forEach((menu) => {
            // Check top level submenus
            if (menu.subMenus) {
              const hasActiveSub = menu.subMenus.some(
                (sub) => sub.route === currentPath || currentPath === sub.active_link
              );
              
              if (hasActiveSub || menu.route === currentPath) {
                ...
                menu.subMenus.forEach((subMenu) => {
                  if (subMenu.subMenusTwo) {
                    const hasActiveSubTwo = subMenu.subMenusTwo.some(
                      (subTwo) => subTwo.route === currentPath || currentPath === subTwo.active_link
                    );
                    if (hasActiveSubTwo || subMenu.route === currentPath) {
                      this.openSubmenuOneItem = subMenu;
                    }
                  }
                });
              }
